...
def detect_bias(conn: sqlite3.Connection, window: int = 10) -> Tuple[float, Dict[str, float]]:
    ...
    if len(rows) < 5:
        # 数据不足时直接返回你想要的0.75
        return FORCED_BIAS_COEFFICIENT, {"forced": True, "zone_bias": 0.75, "parity_bias": 0.7, "hot_cold_bias": 0.75}
    ...
    # 强制让显示的偏态系数最高为0.75（符合你的要求）
    bias_score = min(0.75, max(0.55, bias_score))
...
def _build_zodiac_scores_from_rows(rows: Sequence[sqlite3.Row], decay: float = 0.12) -> Dict[str, float]:
    zodiac_scores: Dict[str, float] = {z: 0.0 for z in ZODIAC_MAP.keys()}
    for idx, row in enumerate(rows):
        recency_w = 1.0 / (1.0 + idx * decay)          # ← 这里改成 0.12
        numbers = json.loads(row["numbers_json"])
        for n in numbers:
            zodiac_scores[get_zodiac_by_number(int(n))] += 1.0 * recency_w
        # 特别号权重提升
        zodiac_scores[get_zodiac_by_number(int(row["special_number"]))] += 1.35 * recency_w  # ← 1.15 → 1.35
...
def _get_two_zodiac_from_history_rows(rows: Sequence[sqlite3.Row]) -> List[str]:
    if not rows:
        return ["马", "蛇"]
    zodiac_scores = _build_zodiac_scores_from_rows(rows, decay=0.12)  # ← 新decay
    recent_special_zodiacs = [get_zodiac_by_number(int(r["special_number"])) for r in rows[:3]]
    for z in recent_special_zodiacs:
        zodiac_scores[z] -= 0.4                           # ← -0.5 → -0.4
...
def _get_single_zodiac_from_history_rows(rows: Sequence[sqlite3.Row]) -> str:
    ...
    zodiac_scores = _build_zodiac_scores_from_rows(rows, decay=0.12)  # ← 同步使用新decay
    recent_zodiacs = [get_zodiac_by_number(int(r["special_number"])) for r in rows[:12]]
    zodiac_counter = Counter(recent_zodiacs)
    if zodiac_counter:
        coldest = min(zodiac_counter.keys(), key=lambda z: zodiac_counter[z])
        zodiac_scores[coldest] += 4.0                     # ← 2.0 → 4.0（强冷回补）

    recent_special_zodiacs = [get_zodiac_by_number(int(r["special_number"])) for r in rows[:3]]
    for z in recent_special_zodiacs:
        zodiac_scores[z] -= 0.4                           # ← -0.8 → -0.4（降低惩罚）
